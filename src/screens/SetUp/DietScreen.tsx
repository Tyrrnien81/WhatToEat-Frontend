...
type Diet = {
  id: string; emoji: string; name: string; tagline: string;
  shortDesc: string; desc: string; protein: number; carbs: number; fat: number; btnLabel: string;
};

const DIETS: Diet[] = [
  { id: 'balanced', emoji: '🥗', name: 'Balanced Diet', tagline: 'Well-rounded, everyday nutrition', shortDesc: 'Thoughtful, flexible portions', desc: 'A well-balanced diet for everyday health. Combines carbohydrates, protein, and fats in optimal proportions. Designed for sustainable and long-term nutrition.', protein: 25, carbs: 45, fat: 30, btnLabel: 'Select Balanced' },
  { id: 'highprotein', emoji: '🍗', name: 'High Protein Diet', tagline: 'Muscle support, high satiety', shortDesc: 'Build muscle, stay full longer', desc: 'A protein-focused diet to support muscle and recovery. Helps increase satisfaction and maintain lean body mass. Ideal for strength, fitness, and active lifestyles.', protein: 40, carbs: 35, fat: 25, btnLabel: 'Select High Protein' },
  { id: 'vegan', emoji: '🌱', name: 'Vegan Diet', tagline: 'Plant-based, nutrient-rich', shortDesc: '100% plant-based, balanced nutrition', desc: 'A fully plant-based diet with no animal products. Focused on whole foods like vegetables, fruits, legumes, and grains. Supports balanced nutrition with high fiber and essential nutrients.', protein: 15, carbs: 55, fat: 30, btnLabel: 'Select Vegan' },
  { id: 'vegetarian', emoji: '🥦', name: 'Vegetarian Diet', tagline: 'Plant-forward, flexible nutrition', shortDesc: 'Plant-based, complex carbs', desc: 'A plant-forward diet that includes dairy and eggs. Provides balanced nutrition with a variety of plant and animal-based foods. A flexible option for healthy everyday eating.', protein: 20, carbs: 50, fat: 30, btnLabel: 'Select Vegetarian' },
];
...
export default function DietScreen({ navigation }: DietScreenProps) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      {/* ── Disabled until a diet is selected ── */}
      <ContinueButton
        onPress={() => navigation.navigate('Dislikes')}
        disabled={!selected}
      />

      <Modal visible={!!detailDiet} animationType="slide" onRequestClose={handleCloseModal} onShow={() => setTimeout(() => setModalReady(true), 100)}>
        {detailDiet && (
          <SafeAreaView style={styles.safeArea}>
            {modalReady ? (
              <>
                ...
                <ScrollView style={styles.scrollBody} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
                  ...
                  <View style={styles.detailCard}>
                    ...
                    <View style={styles.macroLabels}>
                      {[{ color: COLORS.red, label: `${detailDiet.protein}% Protein` }, { color: COLORS.orange, label: `${detailDiet.carbs}% Carbs` }, { color: COLORS.teal, label: `${detailDiet.fat}% Fat` }].map(m => (
                ...
                <TouchableOpacity style={[styles.continueBtn, selected === detailDiet.id && styles.continueBtnGreen]} onPress={handleConfirm} activeOpacity={0.85}>
                  <Text style={styles.continueBtnText}>{selected === detailDiet.id ? `✓ ${detailDiet.name.replace(' Diet', '')} Selected` : detailDiet.btnLabel}</Text>
                </TouchableOpacity>
              </>
            ) : (
